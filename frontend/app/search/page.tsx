...
// APIから受け取るショールームデータの形（フロント表示用に正規化した形）
type Showroom = {
  id: number;
  name: string;
  pref: string;
  ...
  useEffect(() => {
    const fetchShowrooms = async () => {
      try {
        ...
        const normalizedShowrooms: Showroom[] = items.map((item: any) => ({
          id: item.id,
          name: item.name ?? '',
          pref: item.prefecture ?? item.pref ?? '',
          region: item.city ?? item.region ?? '',
          category: item.category ?? '',
          target: item.target ?? '',
          image: item.image_url ?? item.image ?? '/images/showroom-default.jpg',
        ...
        setShowrooms(normalizedShowrooms);
      } catch (err) {
        console.error(err);
                ...
                {showrooms.map((item) => (
                  ...
                  >
                    <div className="flex gap-4 items-center min-w-0">
                      {/* ショールーム画像 */}
                      <img
                        src={item.image || '/images/showroom-default.jpg'}
                        alt={item.name}
